...
const buildDealTitle = (leadData) => {
  const raw = leadData.project || 'Proyecto sin especificar';
  const project = AREA_LABELS[raw] || raw;
  return `${project} - ${leadData.name}`.slice(0, 255);
};

const resolveRouting = (leadData) => {
  const project = (leadData.project || '').toLowerCase();

  if (project.includes('este') && process.env.PIPEDRIVE_PIPELINE_ESTE) {
    return {
      pipeline_id: toNullableNumber(process.env.PIPEDRIVE_PIPELINE_ESTE),
      stage_id: toNullableNumber(process.env.PIPEDRIVE_STAGE_ESTE)
    };
  }

  if (project.includes('oeste') && process.env.PIPEDRIVE_PIPELINE_OESTE) {
    return {
      pipeline_id: toNullableNumber(process.env.PIPEDRIVE_PIPELINE_OESTE),
      stage_id: toNullableNumber(process.env.PIPEDRIVE_STAGE_OESTE)
    };
  }

  // Fallback a variables genéricas
  return {
...
const createDeal = async ({ personId, leadData }) => {
  const routing = resolveRouting(leadData);
  const dealPayload = {
